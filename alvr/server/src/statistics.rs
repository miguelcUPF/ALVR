...
impl StatisticsManager {
    ...
    pub fn new(
        ...
        Self {
            ...
            decoder_queue_delay_average: SlidingWindowAverage::new(Duration::ZERO, max_history_size),
    ...
    pub fn report_frame_encoded(&mut self, target_timestamp: Duration, bytes: usize, is_idr:bool) -> Duration {
            ...
            .find(|frame| frame.server_stats.target_timestamp == target_timestamp)
        {
            ...
            let frame_interval_encode = 
                now.saturating_duration_since(self.prev_encoding);
    ...
    pub fn report_frame_transmitted(&mut self, target_timestamp: Duration, packet_index: u32, 
                             shards_bytes: IndexMap<u16, usize>, shards_instant: IndexMap<u16, Instant> 
                            )  {
            ...
            .find(|frame| frame.server_stats.target_timestamp == target_timestamp)
        {
            ...
            debug!("Frame {} encoded instant {}", packet_index, format!("{:?}", frame.frame_encoded)); // remove
            debug!("Frame {} sent instant {}", packet_index, format!("{:?}", last_instant)); // remove
    ...
    pub fn report_statistics_summary(&mut self){
        ...
        alvr_events::send_event(EventType::StatisticsSummary(StatisticsSummary {
            ...
            frames_sent_per_sec: (self.partial_sum_frames_sent as f32 / interval_secs)
                as _,

            total_frames_received: self.total_frames_received,
            frames_received_per_sec: (self.partial_sum_frames_received as f32 / interval_secs)
                as _,

            total_mbits_sent: (self.total_bytes_sent as f32 / (1e6)) as usize,
            mbits_sent_per_sec: self.partial_sum_bytes_sent as f32 * 8.
                /  (1e6)
                / interval_secs,
            ...
            shards_sent_per_sec: (self.partial_sum_shards_sent as f32 / interval_secs)
                as _,

            total_shards_received: self.total_shards_received,
            shards_received_per_sec: (self.partial_sum_shards_received as f32 / interval_secs)
                as _,

            total_pipeline_latency_average_ms: self.total_pipeline_latency_average.get_average().as_secs_f32() * 1000.,
            game_delay_average_ms: self.game_delay_average.get_average().as_secs_f32() * 1000.,
            server_compositor_delay_average_ms: self.server_compositor_average.get_average().as_secs_f32() * 1000.,
            encode_delay_average_ms: self.encode_delay_average.get_average().as_secs_f32() * 1000.,
            network_delay_average_ms: self.network_delay_average.get_average().as_secs_f32() * 1000.,
            decode_delay_average_ms: self.decode_delay_average.get_average().as_secs_f32() * 1000.,
            decoder_queue_delay_average_ms: self.decoder_queue_delay_average.get_average().as_secs_f32() * 1000.,
            client_compositor_average_ms: self.client_compositor_average.get_average().as_secs_f32() * 1000.,
            vsync_queue_delay_average_ms: self.vsync_queue_delay_average.get_average().as_secs_f32() * 1000.,
    ...
    pub fn report_statistics(&mut self, client_stats: ClientStatistics) -> Duration {
        ...
        let frame = match self.history_buffer.iter_mut().find(|frame| frame.server_stats.target_timestamp == client_stats.target_timestamp) {
        ...
        let highest_frame_received = if frame.server_stats.frame_index == client_stats.highest_frame_index {
            frame.clone()
        } else {
            self.history_buffer.iter_mut().find(|frame_| frame_.server_stats.frame_index == client_stats.highest_frame_index)
                .map_or_else(|| frame.clone(), |frame_searched| frame_searched.clone())
        };

        // Frame statistics
        //// Latency metrics
        let game_delay = frame.frame_presented.saturating_duration_since(frame.tracking_received);
        let rendering = frame.frame_composed.saturating_duration_since(frame.frame_presented);
        let video_encode = frame.frame_encoded.saturating_duration_since(frame.frame_composed);
        ...
        / frame.server_stats.frame_interval_encode
            .max(Duration::from_millis(1))
            .as_secs_f32();
        ...
        if self.is_first_stats {
            let lowest_frame_temp = self.history_buffer
                                        .iter_mut()
                                        .min_by_key(|frame_| frame_.server_stats.frame_index);
            self.prev_last_shard_received_instant = *lowest_frame_temp.unwrap().server_stats.shards_instant.values().min().unwrap();
        ...
        if let Some(instant) = highest_frame_received.server_stats.shards_instant.get(&(client_stats.highest_shard_index as u16)) {
            last_shard_received_instant = *instant;
        }

        let transmission_interval = last_shard_received_instant.saturating_duration_since(self.prev_last_shard_received_instant);

        let shards_sent;
        let bytes_sent;

        if highest_idx == prev_idx {
            shards_sent = client_stats.highest_shard_index.saturating_sub(self.prev_highest_shard_index);

            bytes_sent = highest_frame_received
                .server_stats
                .shards_bytes
                .iter()
                .filter(|&(shard_index, _)| *shard_index > self.prev_highest_shard_index as u16 && *shard_index <= client_stats.highest_shard_index as u16)
                .map(|(_, &bytes)| bytes)
                .sum::<usize>();
        } else {
            // Previous highest frame received
            let shards_from_prev = self.prev_highest_frame
                .server_stats
                .shards_bytes
                .iter()
                .filter(|&(shard_index, _)| *shard_index > self.prev_highest_shard_index as u16)
                .count();
            let bytes_from_prev = self.prev_highest_frame
                .server_stats
                .shards_bytes
                .iter()
                .filter(|&(shard_index, _)| *shard_index > self.prev_highest_shard_index as u16)
            ...
            let shards_in_between = self.history_buffer.iter_mut()
                .filter(|frame| {
                    let frame_idx = frame.server_stats.frame_index;

                    if highest_idx < 257 && prev_idx > std::u32::MAX - 257 {
                        (frame_idx > prev_idx && frame_idx <= std::u32::MAX) || (frame_idx <= highest_idx)
                    } else {
                ...
                .count();
            let bytes_in_between = self.history_buffer.iter_mut()
                .filter(|frame| {
                    let frame_idx = frame.server_stats.frame_index;
        ...
        let mbits_sent_per_sec = (bytes_sent as f32 * 8.) / (1e6) / transmission_interval
                                .max(Duration::from_millis(1))
                                .as_secs_f32();
        let mbits_received_per_sec = (bytes_received as f32 * 8.) / (1e6) / client_stats
                                .frame_interval
                                .max(Duration::from_millis(1))
                                .as_secs_f32();
    ...
    pub fn duration_until_next_vsync(&mut self) -> Duration {
        ...
        (self.prev_vsync_instant + self.nominal_server_frame_interval).saturating_duration_since(now)
    }
}
